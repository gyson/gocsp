...
function handle(generator, ch) {
    do {
        if (!isChannel(ch)) throw new Error("Can only yield Channel type!");
...
function* wait(timeout, generator) {
    ...
    // need to clear timeout if it's on time?
    setTimeout(function() {
...
function* select(channels) {
    for (var index in channels) {
        ...
        if (!isChannel(ch)) throw new Error("select() only handle Channel type!");
...
function* take(channel) {
    if (!isChannel(channel)) {
        throw new Error("take() only handle Channel type");
    }
